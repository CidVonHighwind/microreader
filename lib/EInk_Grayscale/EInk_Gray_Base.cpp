...
EInk_Gray_Base::EInk_Gray_Base(int16_t cs, int16_t dc, int16_t rst, int16_t busy, int16_t busy_level,
                               uint32_t busy_timeout, uint16_t w, uint16_t h, EInk_Gray::Panel p, bool c, bool pu,
                               bool fpu)
    : WIDTH(w),
      HEIGHT(h),
      panel(p),
...
void EInk_Gray_Base::init(uint32_t serial_diag_bitrate) {
  init(serial_diag_bitrate, true, 10, false);
}

void EInk_Gray_Base::init(uint32_t serial_diag_bitrate, bool initial, uint16_t reset_duration, bool pulldown_rst_mode) {
...
void EInk_Gray_Base::setBusyCallback(void (*busyCallback)(const void*), const void* busy_callback_parameter) {
  _busy_callback = busyCallback;
  _busy_callback_parameter = busy_callback_parameter;
}

void EInk_Gray_Base::selectSPI(SPIClass& spi, SPISettings spi_settings) {
  _pSPIx = &spi;
  _spi_settings = spi_settings;
}

void EInk_Gray_Base::_reset() {
  if (_rst >= 0) {
...
void EInk_Gray_Base::_waitWhileBusy(const char* comment, uint16_t busy_time) {
  if (_busy >= 0) {
...
void EInk_Gray_Base::_writeCommand(uint8_t c) {
...
void EInk_Gray_Base::_writeData(uint8_t d) {
  _pSPIx->beginTransaction(_spi_settings);
  if (_cs >= 0)
    digitalWrite(_cs, LOW);
  _pSPIx->transfer(d);
...
void EInk_Gray_Base::_writeData(const uint8_t* data, uint16_t n) {
  _pSPIx->beginTransaction(_spi_settings);
  if (_cs >= 0)
    digitalWrite(_cs, LOW);
...
void EInk_Gray_Base::_writeDataPGM(const uint8_t* data, uint16_t n, int16_t fill_with_zeroes) {
  _pSPIx->beginTransaction(_spi_settings);
  if (_cs >= 0)
    digitalWrite(_cs, LOW);
  for (uint16_t i = 0; i < n; i++) {
...
void EInk_Gray_Base::_writeDataPGM_sCS(const uint8_t* data, uint16_t n, int16_t fill_with_zeroes) {
  _pSPIx->beginTransaction(_spi_settings);
...
void EInk_Gray_Base::_writeCommandData(const uint8_t* pCommandData, uint8_t datalen) {
  _pSPIx->beginTransaction(_spi_settings);
  if (_dc >= 0)
    digitalWrite(_dc, LOW);
  if (_cs >= 0)
...
void EInk_Gray_Base::_writeCommandDataPGM(const uint8_t* pCommandData, uint8_t datalen) {
  _pSPIx->beginTransaction(_spi_settings);
  if (_dc >= 0)
    digitalWrite(_dc, LOW);
  if (_cs >= 0)
...
void EInk_Gray_Base::_startTransfer() {
  _pSPIx->beginTransaction(_spi_settings);
  if (_cs >= 0)
    digitalWrite(_cs, LOW);
}

void EInk_Gray_Base::_transfer(uint8_t value) {
  _pSPIx->transfer(value);
}

void EInk_Gray_Base::_endTransfer() {
  if (_cs >= 0)
    digitalWrite(_cs, HIGH);
  _pSPIx->endTransaction();
}

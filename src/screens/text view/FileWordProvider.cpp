...
String FileWordProvider::scanWord(int direction, TextRenderer& /*renderer*/) {
  prevIndex_ = index_;
...
String FileWordProvider::getNextWord(TextRenderer& renderer) {
  return scanWord(+1, renderer);
}
String FileWordProvider::getPrevWord(TextRenderer& renderer) {
  return scanWord(-1, renderer);
}
